...
class TwoStreamDataset(Dataset):
    """Appearance + Flow Features for Attention Level Dataset.
    Args:
    ...
    def __getitem__(self, idx):
        ...
        X_app.append(cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB))
        frame1 = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)
        for i in range(self.sequence_len-1):
            _, frame2 = self.cap.read()
            frame2 = cv2.resize(frame2, (256,256))
            X_app.append(cv2.cvtColor(frame2, cv2.COLOR_BGR2RGB))
            ...
            X_flow.append(np.stack((flow[:,:,0], flow[:,:,1], mag),axis=2))
            frame1 = frame2

        # convert to numpy array
        X_app = np.array(X_app)
        X_flow = np.array(X_flow)
        # remove first appearance feature
        X_app = X_app[1:]
        # transform data
        if self.transform:
            X_app = self.transform(X_app)
            X_flow = self.transform(X_flow)

        # reformat [numSeqs x numChannels x Height x Width]
        X_app = np.transpose(X_app, (0,3,1,2))
        X_flow = np.transpose(X_flow, (0,3,1,2))
        # store in sample
        sample = {'X': (X_app, X_flow), 'y': y}
        return sample
...
class Normalize():
    ...
    this transform will normalize each channge of the input video.
...
def get_loaders(train_path, valid_path, batch_size, sequence_len, num_workers, 
        gpu):
    ...
    datasets = {
            'Train': TwoStreamDataset(train_path, sequence_len,
                data_transforms['Train']),
            'Valid': TwoStreamDataset(valid_path, sequence_len,
...
def main():
    ...
    # hyper-parameters
    batch_size = 2
    sequence_len = 25  # counting backwards from last frame
    
    dataloaders, dataset_sizes = get_loaders(train_path, valid_path, 
            batch_size, sequence_len, num_workers=0, gpu=False)
    print('Dataset Sizes:')
    print(dataset_sizes)
    ...
    grid = utils.make_grid(data[0][0])
    imshow(grid)
    grid = utils.make_grid(data[1][0])
    imshow(grid)
    plt.show()
