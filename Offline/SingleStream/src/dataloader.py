...
class AppearanceDataset(Dataset):
    ...
    """
    def __init__(self, labels_path, sequence_len, sample_rate, transform=None):
...
def get_loaders(train_path, valid_path, batch_size, sequence_len,
        sample_rate, flow, num_workers, gpu):
    ...
    Args:
        ...
        sequence_len (int):     number of frames to keep in video (counting
                                    from last frame)
        sample_rate (int):    sample video sequence
    ...
    if flow:
        datasets = {
                'Train': FlowDataset(train_path, sequence_len,
                    sample_rate, data_transforms['Train']),
                'Valid': FlowDataset(valid_path, sequence_len,
                    sample_rate, data_transforms['Valid'])
                }
    else:
        datasets = {
...
def main():
    ...
    dataloaders, dataset_sizes = get_loaders(train_path, valid_path, 
            batch_size, sequence_len, sample_rate, flow, num_workers=0, 
            gpu=False)
    print('Dataset Sizes:')
    print(dataset_sizes)
